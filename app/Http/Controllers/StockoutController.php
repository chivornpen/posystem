...
//
                ...
                foreach ($result as $r){
                    $res = DB::table('import_product')->select('id', 'qty', 'importId','productId', 'mfd', 'expd')->where([['importId','=',$r->importId],['productId','=',$product_id],['qty','>',0],['expd','>',$now],])->get();//select one by one from import
                        if($res) {
                            foreach ($res as $s) {
                                $qt = $qt;
