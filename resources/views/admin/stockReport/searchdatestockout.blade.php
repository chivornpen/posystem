          ...
          <div style="margin-top: 5px;margin-bottom: 5px;font-size: 12px;font-family: 'Khmer OS System';">Reported By: <b>{{Auth::user()->nameDisplay}}</b></div>
          <table width="1300px" class="table-responsive" border="1px" style="border: 1px solid gray; border-collapse: collapse;" cellpadding="5px" cellspacing="0">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style="text-align: center;padding-left: 3px;font-size: 10px;height: 20px; font-family: 'Arial';">{{Carbon\Carbon::parse($out->stockoutDate)->format('d-M-Y')}}</td>
                <td style="text-align: center;font-size: 10px; height: 20px; font-family: 'Arial';">
                  <?php 
                        echo "CAM-IN-" . sprintf('%06d',$out->purchaseorder->id);
                  ?>
                </td>
                ...
                @foreach($products as $pro)
                  ...
                  $product_id =0;
                  $qty =0;
                      $purchaseorders = DB::table('purchaseorder_product')->where([['purchaseorder_id','=',$out->purchaseorder_id],['product_id','=',$pro->id],])->get();
