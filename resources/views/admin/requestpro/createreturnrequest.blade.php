                    ...
                    <div class="col-lg-6">
                        <div class="form-group">
                            <select name="return" id="returnInvId" onchange="viewInvoice()" class="form-control">
                                ...
                                @foreach( $stockoutres as $invN)
                                    <option value="{{$invN->requestpro_id}}">{!! "CAM-REQ-" . sprintf('%06d',$invN->requestpro_id) !!}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
       ...
       function Return(importId, productID, expd, qty,stockoutId) {
        ...
        function checkQty() {
            ...
            if( parseInt(Qty) < qty || qty<=0){
                ...
                $("#returnQty").css('border','');
                $('#btn_save').removeAttr('disabled','true');
                err="";
            }
        ...
        function SaveReturn() {
            ...
            if(err==""){
                $.ajax({
                    type: 'get',
                    url: "{{url('returnsomereqpro')}}"+"/"+stId+"/"+Qty+"/"+qty+"/"+proId+"/"+impId+"/"+returnBy+"/"+InvN,
                    dataType: 'html',
                    success:function (data) {
                        $("#ReturnInvoice").html(data);
