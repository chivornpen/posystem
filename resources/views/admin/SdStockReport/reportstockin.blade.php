            ...
            <tbody>
              ...
              <tr>
                ...
                <td style="text-align: center;font-size: 10px; height: 20px; font-family: 'Arial';">
                  <?php 
                        echo "CAM-IN-" . sprintf('%06d',$in->purchaseorder_id);
                  ?>
                </td>
                <td style="text-align: center;font-size: 10px;height: 20px; font-family: 'Arial';">{{Carbon\Carbon::parse($in->subimportDate)->format('d-M-Y')}}</td>
                ...
                @if(Auth::user()->position->name!='SD')
                <td style="padding-left: 3px;font-size: 10px;height: 20px; font-family: 'Khmer OS System';">{{$in->brand->brandName}}</td>
                @endif
                @foreach($brandProducts as $brand_pro)
                  <?php 
                  $product_id =0;
                  $qty =0;
                      $purchaseorders = DB::table('purchaseorder_product')->where([['purchaseorder_id','=',$in->purchaseorder_id],['product_id','=',$brand_pro->product_id],])->get();
...
//-----------------------
    function saerch() {
              ...
              if(brand_id ==0 &&startDate=='' && endDate ==''){
                ...
                $.ajax({
                  type : 'get',
                  url : "{{url('/searchIn')}}"+"/"+0+"/"+startDate+"/"+endDate,
                  dataType: 'html',
                  success:function (data) {
                    $('.startdate').css('border','1px solid lightblue');
                    $('.enddate').css('border','1px solid lightblue');
                ...
                $.ajax({
                  type : 'get',
                  url : "{{url('/searchIn')}}"+"/"+brand_id+"/"+startDate+"/"+endDate,
                  dataType: 'html',
                  success:function (data) {
                    $('.startdate').css('border','1px solid lightblue');
                    $('.enddate').css('border','1px solid lightblue');
