                        ...
                        <div class="col-lg-12">
                            <select name="invoice" id="invoice" class="form-control" onchange="listInvoice()">
                                ...
                                @foreach($exchange as $ex)
                                    @if($ex->purchaseorder_id=="")
                                        <option value="{{$ex->id}}">{{"CAM-IN-".sprintf('%06d',$ex->stockout->purchaseorder_id)}}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
    ...
    <script type="text/javascript">
        function listInvoice() {
            var exId = $("#invoice").val();
                ...
                $.ajax({
                    type: 'get',
                    url: "{{url('invoicePO/VUXchangeInvoice/')}}"+"/"+exId,
                    dataType: 'html',
