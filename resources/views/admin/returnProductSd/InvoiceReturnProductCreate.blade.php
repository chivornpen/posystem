                    ...
                    <div class="col-lg-6">
                        <select name="Rinvoice" id="Rinvoice" class="form-control">
                            ...
                            @foreach($productReturn as $re)
                                    <option value="{{$re->id}}">{{"CAM-IN-".sprintf('%06d',$re->stockoutsd->purchaseordersd_id)}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-lg-6">
    ...
    <script type="text/javascript">
            function productInvoice() {
                ...
                if(returnId!=0 && status!=0){
                    $.ajax({
                        type: 'get',
                        url: "{{url('/showContentInvReturn')}}"+"/"+returnId+"/"+status,
                        dataType: 'html',
            ...
            function ProductReturCreate(returnId) {
                ...
                $.ajax({
                   type: 'get',
                    url: "{{url('/productReturnCreateInv')}}"+"/"+returnId+"/"+status,
                    dataType: 'html',
                    success:function (data) {
                        location.reload();
                    },
